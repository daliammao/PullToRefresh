package com.daliammao.ptr.footer;
...
import com.daliammao.ptr.utils.TimeUtil;
...
public class PtrClassicDefaultFooter extends FrameLayout implements PtrUIFooterHandler {
    ...
    protected void initViews(AttributeSet attrs) {
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.PtrClassicFooter, 0, 0);
        if (arr != null) {
            mRotateAniTime = arr.getInt(R.styleable.PtrClassicFooter_ptr_load_rotate_ani_time, mRotateAniTime);
        }
        buildAnimation();
        View footer = LayoutInflater.from(getContext()).inflate(R.layout.ptr_classic_default_header, this);

        mRotateView = footer.findViewById(R.id.ptr_classic_header_rotate_view);
        mRotateView.setRotation(180);

        mTitleTextView = (TextView) footer.findViewById(R.id.ptr_classic_header_rotate_view_header_title);
        mLastUpdateTextView = (TextView) footer.findViewById(R.id.ptr_classic_header_rotate_view_header_last_update);
        mProgressBar = footer.findViewById(R.id.ptr_classic_header_rotate_view_progressbar);
    ...
    private String getLastUpdateTime() {
        ...
        return TimeUtil.getFriendlyUpdateTime(getContext(), mLastUpdateTime);
    }

    @Override
    public void onUIPositionChange(PtrFrameLayout frame, boolean isUnderTouch, byte status, PtrIndicator ptrIndicator) {

        final int mOffsetToLoad = ptrIndicator.getOffsetToLoad();
        ...
        if (currentPos > -mOffsetToLoad && lastPos <= -mOffsetToLoad) {
            if (isUnderTouch && status == PtrFrameLayout.PTR_STATUS_PREPARE_LOAD) {
